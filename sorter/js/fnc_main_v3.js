...
function init() {
  ...
  for (i = 0; i < ary_SortData.length; i++) {
    ...
    if (ary_SortData[i].length >= 2) {
      ...
      ary_SortData[int_Pointer] = ary_SortData[i].slice(
        int_Marker,
        ary_SortData[i].length
      );
      int_Total += ary_SortData[int_Pointer].length;
      ary_ParentData[int_Pointer] = i;
      int_Pointer++;
...
function imginit() {
  ...
  gID("lbl_imgParcent").innerHTML = Math.floor(
    (int_ImgValue / int_ImgMax) * 100
  );
...
function fnc_Sort(int_SelectID) {
  ...
  // 引き分けの場合のみ
  if (int_SelectID == 0) {
    ary_EqualData[ary_RecordData[int_RecordID - 1]] =
      ary_SortData[int_RightList][int_RightID];
  }
  ...
  // 片方のリストを走査し終えた後の処理
  if (
    int_LeftID < ary_SortData[int_LeftList].length &&
    int_RightID == ary_SortData[int_RightList].length
  ) {
  ...
  if (
    ...
    for (
      i = 0;
      i <
      ary_SortData[int_LeftList].length + ary_SortData[int_RightList].length;
      i++
    ) {
...
function fnc_CountUp(int_Select) {
  ary_RecordData[int_RecordID] =
    ary_SortData[int_Select == 0 ? int_LeftList : int_RightList][
      int_Select == 0 ? int_LeftID : int_RightID
    ];
...
function fnc_ShowData() {
  ...
  if (int_Status == 2) {
      ...
      if (int_ResultImg != 0 && i < int_ResultRank) {
        ...
        if (obj_TempData[3].length > 0) {
          new_img.src =
            str_ImgPath +
            obj_TempData[
              Math.floor(Math.random() * (obj_TempData.length - 3)) + 3
            ];
          new_cell.appendChild(new_img);
          new_cell.appendChild(cE("br"));
          bln_imgFlag = true;
    ...
    for (i = 0; i < 2; i++) {
      ...
      var obj_TempData =
        ary_TempData[
          ary_SortData[i == 0 ? int_LeftList : int_RightList][
            i == 0 ? int_LeftID : int_RightID
          ]
        ];
