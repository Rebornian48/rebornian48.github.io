...
function fCG(sID, iGM, iGT) {
  ...
  for (iG = 0; iG < iGM; iG++) {
    if (iG % 10 == 0) {
      sCN = iG < iGT ? "leftend" : "leftendr";
    } else if (iG % 5 == 0) {
      sCN = iG < iGT ? "mid" : "midr";
    } else {
      sCN = iG < iGT ? "normal" : "normalr";
    }
...
function eGR(sID, iNow) {
  for (i = 0; i < iGM; i++) {
    // �F�Q�[�W
    if (i < iNow) {
      gID(sID + i).style.backgroundColor = "#9999ff";
    } else {
      gID(sID + i).style.backgroundColor = "#ffffff";
    }
  }
}
...
// # return - number
function fC2N(c) {
  if (0x30 <= c.charCodeAt(0) && c.charCodeAt(0) < 0x3a) {
    return c.charCodeAt(0) - 0x30;
  } else if (0x41 <= c.charCodeAt(0) && c.charCodeAt(0) <= 0x5a) {
    return c.charCodeAt(0) - 0x41 + 10;
  } else if (0x61 <= c.charCodeAt(0) && c.charCodeAt(0) <= 0x78) {
    return c.charCodeAt(0) - 0x61 + 36;
  }
...
function fN2C(n) {
  if (n < 10) {
    ...
    return String.fromCharCode(0x41 + n - 10);
  } else if (n < 60) {
    return String.fromCharCode(0x61 + n - 36);
  } else if (n < 100) {
    return "z" + fN2C(n - 60);
  }
